...
# create widget text db_source default "mh_pre_v5_clear";
...
# DBTITLE 1,CREATES STANDARDISED RATES FOR EACH ETHNIC / GENDER / AGE GROUP
 ...
 LEFT OUTER JOIN global_temp.ONS_Population p
  ON CRS.Der_Gender = p.Der_Gender
  AND CRS.Age_group = p.Age_Group
  AND CRS.ethnic_group_formatted = p.ethnic_group_formatted;
...
# DBTITLE 1,CREATES STANDARDISED RATES PER 100,000 POPULATION FOR EACH ETHNIC GROUP
 ...
 SELECT			SRS.ethnic_group_formatted
                 ,SUM(SRS.People) AS PEOPLE
                 ,SUM(SRS.Population) as POPULATION
...
# DBTITLE 1,Pulls together confidence intervals for each Ethncity
 ...
 SELECT CIINT.ethnic_group_formatted
   ...
   ,1.96*SQRT(SUM(CI_rate))*100000 as CONFIDENCE_INTERVAL_95_TEST
...
# COMMAND ----------
 ...
 %sql
 select * from $db_output.NHSDEthnicityDim
