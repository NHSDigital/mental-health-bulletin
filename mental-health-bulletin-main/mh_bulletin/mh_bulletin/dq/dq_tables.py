...
startchoices = [str(r[0]) for r in spark.sql("select distinct ReportingPeriodStartDate from $mhsds_db.mhs000header order by ReportingPeriodStartDate").collect()]
endchoices = [str(r[0]) for r in spark.sql("select distinct ReportingPeriodEndDate from $mhsds_db.mhs000header order by ReportingPeriodEndDate").collect()]
monthid = [str(r[0]) for r in spark.sql("select distinct Uniqmonthid from $mhsds_db.mhs000header order by Uniqmonthid").collect()]
...
# DBTITLE 1,DQ Table 2 Final - Download this output
# data = sqlContext.sql(""" select  r.orgidprov AS ORGANISATION_CODE,
#                                    od.Name AS ORGANISATION_NAME,
...
#                                         END AS YEAR,
#                                    CONCAT(CAST(YEAR(h.ReportingPeriodStartDate) AS VARCHAR(4)),RIGHT(CONCAT('00',MONTH(h.ReportingPeriodStartDate)),2)) AS ReportingPeriodStartDate,
#                                    count (distinct r.person_id) as COUNT
#                           from """+dbutils.widgets.get("dbm")+""".mhs101referral r
#                           left join """+dbutils.widgets.get("dbm")+""".mhs000header h on h.uniqmonthid = r.uniqmonthid
#                           LEFT JOIN """+dbutils.widgets.get("db_output")+""".MHB_ORG_DAILY as od
#                                      ON COALESCE(r.orgidprov, 'UNKNOWN') = COALESCE(od.ORG_CODE) AND od.BUSINESS_END_DATE IS NULL
#                           where r.uniqmonthid  between """+dbutils.widgets.get("start_month_id")+""" and """+dbutils.widgets.get("end_month_id")+"""
#                                and (r.ServDischDate IS NULL OR r.ServDischDate > h.ReportingPeriodEndDate)
#                           group by r.orgidprov, 
#                                    od.Name,
#                                    CONCAT(CAST(YEAR(h.ReportingPeriodStartDate) AS VARCHAR(4)),RIGHT(CONCAT('00',MONTH(h.ReportingPeriodStartDate)),2)),
#                                    CASE WHEN MONTH(h.ReportingPeriodStartDate)>=4
#                                         THEN CONCAT(CAST(YEAR(h.ReportingPeriodStartDate) AS VARCHAR(4)),"-",RIGHT(CAST(YEAR(h.ReportingPeriodStartDate)+1 AS VARCHAR(4)),2))
#                                         ELSE CONCAT(CAST(YEAR(h.ReportingPeriodStartDate)-1 AS VARCHAR(4)),"-",RIGHT(CAST(YEAR(h.ReportingPeriodStartDate) AS VARCHAR(4)),2))
#                                         END""")

# data2 = sqlContext.sql(""" select  r.orgidprov AS ORGANISATION_CODE,
#                                    od.Name AS ORGANISATION_NAME,
#                                    CASE WHEN MONTH(h.ReportingPeriodStartDate)>=4
#                                         THEN CONCAT(CAST(YEAR(h.ReportingPeriodStartDate) AS VARCHAR(4)),"-",RIGHT(CAST(YEAR(h.ReportingPeriodStartDate)+1 AS VARCHAR(4)),2))
#                                         ELSE CONCAT(CAST(YEAR(h.ReportingPeriodStartDate)-1 AS VARCHAR(4)),"-",RIGHT(CAST(YEAR(h.ReportingPeriodStartDate) AS VARCHAR(4)),2))
#                                         END AS YEAR,
#                                    CONCAT(CAST(YEAR(h.ReportingPeriodStartDate)+1 AS VARCHAR(4)),RIGHT(CONCAT('00',MONTH(h.ReportingPeriodStartDate)),2)) AS ReportingPeriodStartDate,
#                                    count (distinct r.person_id) as COUNT
#                           from """+dbutils.widgets.get("dbm")+""".mhs101referral r
#                           left join """+dbutils.widgets.get("dbm")+""".mhs000header h on h.uniqmonthid = r.uniqmonthid
#                           LEFT JOIN """+dbutils.widgets.get("db_output")+""".MHB_ORG_DAILY as od
#                                      ON COALESCE(r.orgidprov, 'UNKNOWN') = COALESCE(od.ORG_CODE) AND od.BUSINESS_END_DATE IS NULL
#                           where r.uniqmonthid  between """+dbutils.widgets.get("start_month_id")+"""-12 and """+dbutils.widgets.get("end_month_id")+"""-12
#                                and (r.ServDischDate IS NULL OR r.ServDischDate > h.ReportingPeriodEndDate)
#                           group by r.orgidprov,
#                                    od.Name,
#                                    CONCAT(CAST(YEAR(h.ReportingPeriodStartDate)+1 AS VARCHAR(4)),RIGHT(CONCAT('00',MONTH(h.ReportingPeriodStartDate)),2)),
#                                    CASE WHEN MONTH(h.ReportingPeriodStartDate)>=4
#                                         THEN CONCAT(CAST(YEAR(h.ReportingPeriodStartDate) AS VARCHAR(4)),"-",RIGHT(CAST(YEAR(h.ReportingPeriodStartDate)+1 AS VARCHAR(4)),2))
#                                         ELSE CONCAT(CAST(YEAR(h.ReportingPeriodStartDate)-1 AS VARCHAR(4)),"-",RIGHT(CAST(YEAR(h.ReportingPeriodStartDate) AS VARCHAR(4)),2))
#                                         END""")

# dq2_patients_cur = data.groupby("ORGANISATION_CODE","ORGANISATION_NAME","YEAR").pivot("ReportingPeriodStartDate").sum("COUNT")
# dq2_patients_prev = data2.groupby("ORGANISATION_CODE","ORGANISATION_NAME","YEAR").pivot("ReportingPeriodStartDate").sum("COUNT")
# dq2_patients = dq2_patients_cur.union(dq2_patients_prev).sort("ORGANISATION_NAME","YEAR")
# display(dq2_patients)
# # display(dq2_patients_prev)

# COMMAND ----------

# DBTITLE 1,DQ Table 2 Final (LEGACY)
# %sql
# DROP TABLE IF EXISTS $db_output.DQ_T2_Full;
# CREATE TABLE         $db_output.DQ_T2_Full USING DELTA AS

# select  r.orgidprov AS ORGANISATION_CODE,
#          od.Name AS ORGANISATION_NAME,
#          CASE WHEN MONTH(h.ReportingPeriodStartDate)>=4
#               THEN CONCAT(CAST(YEAR(h.ReportingPeriodStartDate) AS VARCHAR(4)),"-",RIGHT(CAST(YEAR(h.ReportingPeriodStartDate)+1 AS VARCHAR(4)),2))
#               ELSE CONCAT(CAST(YEAR(h.ReportingPeriodStartDate)-1 AS VARCHAR(4)),"-",RIGHT(CAST(YEAR(h.ReportingPeriodStartDate) AS VARCHAR(4)),2))
...
# from (  select *
#         from $dbm.mhs101referral
#         where uniqmonthid  between $start_month_id and $end_month_id) r
# left join $dbm.mhs000header h on h.uniqmonthid = r.uniqmonthid
# LEFT JOIN (   select *
#               from $db_output.MHB_ORG_DAILY
#               where ORG_OPEN_DATE <= '$rp_enddate'
#                 AND ((ORG_CLOSE_DATE >= '$rp_enddate') OR ORG_CLOSE_DATE is NULL)
#                 AND BUSINESS_END_DATE IS NULL) as od
#            ON COALESCE(r.orgidprov, 'UNKNOWN') = COALESCE(od.ORG_CODE)
# --            AND od.ORG_OPEN_DATE <= '$rp_enddate'
# --            AND ((od.ORG_CLOSE_DATE >= '$rp_startdate') OR od.ORG_CLOSE_DATE is NULL)
# where --r.uniqmonthid  between $start_month_id and $end_month_id
#      /*and*/ (r.ServDischDate IS NULL OR r.ServDischDate > h.ReportingPeriodEndDate)
# group by r.orgidprov, 
#          od.Name,
#          CASE WHEN h.ReportingPeriodStartDate not between '$rp_startdate' and '$rp_enddate'
...
# from (  select *
#         from $dbm.mhs101referral
#         where uniqmonthid  between $start_month_id-12 and $end_month_id-12) r
# left join $dbm.mhs000header h on h.uniqmonthid = r.uniqmonthid
# LEFT JOIN (   select *
#               from $db_output.MHB_ORG_DAILY
#               where ORG_OPEN_DATE <= '$rp_enddate'
...
# DBTITLE 1,Convert Spark Dataframes to Pandas Dataframes to create excel spreadsheet
# df_dq1_countprov = dq1_countprov.toPandas()
# dq1_prov = spark.createDataFrame(dq1_prov)
# df_dq1_prov = dq1_prov.toPandas()
# df_dq2_patients = dq2_patients.toPandas()
# df_dq2_provcount = dq2_provcount.toPandas()
# df_dq2_national = dq2_national.toPandas()
# df_dq3 = dq3.toPandas()
# df_dq3_total = dq3_total.toPandas()
